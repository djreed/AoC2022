...
while queue:
    ...
    for newR, newC in [(row + 1, col), (row - 1, col), (row, col + 1), (row, col - 1)]:
        ...
        if (newR, newC) in visited: # If we've already visited the target cell on this path
            continue
        if ord(grid[newR][newC]) - ord(grid[row][col]) > 1: # If the height of the next is more than one higher
            continue
        if newR == endR and newC == endC: # Found the end
            print(dist + 1)
            exit(0)
